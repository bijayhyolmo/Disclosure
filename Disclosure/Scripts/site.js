(function ($) {
    ...
    var App = {
        ...
        events_handler: function () {
            ...
            $("[data-load-views] a:not('.external'), a.js-tabcollapse-panel-heading:not('.external') ").on('click', function (e) {
                ...
                if (checkEmptyRepo) {
                    ...
                    if (alternative) {
                        var repo = $("[data-view=" + alternative + "]").attr("href");
                        if ($(repo).children().length <= 0 || $(repo).is(":hidden") || !$(repo).is(":visible")) {
                            $(repo).parent().children().removeClass("active");
                            $(repo).addClass("active");
                            var deferred = $.Deferred();
                            App.populateTab(alternative, repo, deferred);
                            $.when(deferred).then(function () {
                                scrollToSection(tabRepo);
                            });
                        } else {
                            scrollToSection(tabRepo);
                        }
        ...
        populateTab: function (name, repo, deferred) {
